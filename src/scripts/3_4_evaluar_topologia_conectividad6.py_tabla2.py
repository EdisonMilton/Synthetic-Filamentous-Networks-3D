#07_evaluar_topologia_pred_vs_gt_conectividad6.py
import os
import numpy as np
import pandas as pd
...
OUTPUT_CSV = os.path.join(DATASET_DIR, "tabla2_topologia_pred_vs_gt_conectividad6.csv")

print("DATASET_DIR =", DATASET_DIR)
print("Existe DATASET_DIR:", os.path.exists(DATASET_DIR))

# =========================================================
# KERNEL 3D PARA CONECTIVIDAD 6
# =========================================================
kernel6 = np.zeros((3, 3, 3), dtype=np.uint8)
kernel6[1, 1, 0] = 1
kernel6[1, 1, 2] = 1
kernel6[1, 0, 1] = 1
kernel6[1, 2, 1] = 1
...
def skeleton_from_mask_3d_slice_by_slice(mask):
    """
    Genera skeleton 3D aplicando skeletonize 2D en cada corte.
    Esto mantiene consistencia con tu pipeline anterior.
    """
    mask = (mask > 0)
...
def analizar_skeleton_conectividad6(skel: np.ndarray):
    skel = (skel > 0).astype(np.uint8)

    vecinos = convolve(skel, kernel6, mode="constant", cval=0)
...
for sample in samples:
    ...
    try:
        ...
        pred = analizar_skeleton_conectividad6(pred_skel)
        gt = analizar_skeleton_conectividad6(gt_skel)
